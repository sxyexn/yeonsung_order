// public/admin/admin.js (최종 버전: 완료 페이지 로직 완성)
...
    // Socket.IO 연결은 payment/serving/completed 페이지에서만 수행
    if (document.body.classList.contains('payment-page') || 
        document.body.classList.contains('serving-page') || 
        document.body.classList.contains('completed-page')) { // ✅ completed-page 포함
...
    if (document.body.classList.contains('payment-page')) {
        fetchOrders(); // ✅ 페이지 로드 시 항상 모든 주문을 불러옴 (데이터 유지)
...
        fetchServingItems(); // ✅ 페이지 로드 시 항상 서빙 대기 메뉴를 불러옴 (데이터 유지)
...
    // 4. ✅ 완료 페이지 로직 (completed.html) 추가
    if (document.body.classList.contains('completed-page')) {
        fetchCompletedOrders(); // ✅ 페이지 로드 시 완료된 주문 목록 로드
    }
});
...
        renderOrders(confirmedOrders, 'confirmed-orders-tbody'); // ✅ 입금 확인된 주문도 항상 렌더링
        
...
// ==========================================================
// 4. 완료된 주문 페이지 로직 (completed.html)
// ==========================================================
...
async function fetchCompletedOrders() {
    try {
        ...
        if (!response.ok) throw new Error('완료 주문 데이터를 가져오는 데 실패했습니다.');
        const data = await response.json();
        
        // 서버에서 이미 필터링되어 왔으므로, 별도의 클라이언트 필터링 필요 없음
        const completedOrders = data.orders; 
        
        renderCompletedOrders(completedOrders, 'completed-orders-tbody');
        document.getElementById('completed-count').textContent = completedOrders.length;

    } catch (error) {
        console.error('완료 주문 API 통신 오류:', error);
    }
}

// ✅ 새로운 렌더링 함수 추가 (완료 페이지 전용)
function renderCompletedOrders(orders, tbodyId) {
    const tbody = document.getElementById(tbodyId);
    tbody.innerHTML = '';
